...
# 2 --- CONEXÃO INTELIGENTE (SEM DEPENDER DE NOME) ---
try:
    # Tenta conectar sem especificar nome de aba. 
    # Ele vai pegar automaticamente a primeira aba (Índice 0)
    # ttl=0 garante que ele não use memória antiga (Cache)
    df = conn.read(ttl=0)
    
    st.toast("Dados atualizados com sucesso!", icon="✅") # Aviso visual discreto
    ...
    st.write("O erro técnico exato é:")
    st.code(e) # Isso vai nos mostrar se o erro é Senha, Link ou outra coisa
    st.stop()
